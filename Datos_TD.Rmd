...
```{r mexVsCAN, message=FALSE, eval=T}
...
tb %>% filter(country %in% c("Mexico", "South Africa")) %>% na.omit() %>% group_by(country, sex) %>% summarise(Total = sum(child)) %>% ggplot(mapping=aes(x=sex, y=Total))+geom_col(aes(fill=country))
```
...
```{r}
tb %>% filter(country == "Mexico") %>% mutate(total_casos = child + adult + elderly) %>% group_by(sex, country) %>% summarise(total_casos = sum(total_casos, na.rm = TRUE)) %>% ggplot(aes(x = sex, y = total_casos, fill = country)) + geom_col()

```
- El número total de casos en Afganistan en el 2002.
```{r}
tb %>% filter(country == "Afghanistan", year == 2002) %>% mutate(total_casos =child + adult + elderly) %>% group_by(sex, country) %>% summarise(total_casos = sum(total_casos, na.rm = TRUE)) %>% ggplot(aes(x = sex, y = total_casos, fill = country)) + geom_col()
```
...
tb %>%
  ...
  mutate(total_casos = child + adult + elderly) %>%
  summarise(total_casos_total = sum(total_casos, na.rm = TRUE))
```
